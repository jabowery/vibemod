        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for line in lines:
                if 'def existing_method' in line:
                    indent = len(line) - len(line.lstrip())
                    assert indent == 4, f"Method def should have 4 spaces, got {indent}\nLine: {line!r}"
                elif '"""Docstring."""' in line:
                    indent = len(line) - len(line.lstrip())
                    assert indent == 8, f"Docstring should have 8 spaces, got {indent}\nLine: {line!r}"
                elif 'x = 1' in line:
                    indent = len(line) - len(line.lstrip())
                    assert indent == 8, f"Body statement should have 8 spaces, got {indent}\nLine: {line!r}"
                elif 'if x:' in line:
                    indent = len(line) - len(line.lstrip())
                    assert indent == 8, f"If statement should have 8 spaces, got {indent}\nLine: {line!r}"
                elif 'y = 2' in line:
                    indent = len(line) - len(line.lstrip())
                    assert indent == 12, f"Nested statement should have 12 spaces, got {indent}\nLine: {line!r}"
                elif 'return y' in line:
                    indent = len(line) - len(line.lstrip())
                    assert indent == 8, f"Return should have 8 spaces, got {indent}\nLine: {line!r}"
