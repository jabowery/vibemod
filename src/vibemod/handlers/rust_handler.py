...
def normalize_target_path(self, target_path: str, content: str = None) -> str:
    ...
    Handles cases where user specifies:
    - "fn foo()" or "fn foo" -> "foo"
    - "struct Foo" -> "Foo"
    - "enum Bar" -> "Bar"
    - "const FOO" -> "FOO"
    - "static BAR" -> "BAR"
    ...
    If content is provided, validates that the prefix matches the content type.
    """
    target = target_path.strip()

    # Map of keyword prefixes to their normalized form
    # None means just strip the prefix and use the name
    # 'impl' means convert to impl: syntax
    simple_prefixes = ['fn ', 'struct ', 'enum ', 'const ', 'static ', 
    ...
    for prefix in simple_prefixes:
        if target.lower().startswith(prefix.lower()):
            ...
            # Strip parentheses and parameters if present (e.g., "foo()" -> "foo")
            if '(' in name:
                name = name[:name.index('(')].strip()
            # Strip generics if present (e.g., "Foo<T>" -> "Foo")
            if '<' in name:
                name = name[:name.index('<')].strip()
            return name

    # Handle "impl Type" or "impl Trait for Type"
